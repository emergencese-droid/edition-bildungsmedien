...
-- These are public reference records for a civilian data model only.
-- They do not represent confirmed ETOS members, partners, contracts, or endorsements.
...
VALUES
('AI','Künstliche Intelligenz','Methoden für maschinelles Lernen, Analyse und verantwortungsvolle KI.'),
...
('KNOWLEDGE','Wissenssysteme','Wissensmodelle, Informationsarchitekturen und semantische Verknüpfung.'),
('EDUCATION','Bildung','Lernsysteme, wissenschaftliche Kommunikation und Bildungsmedien.'),
('SUSTAINABILITY','Nachhaltigkeit','Ressourceneffizienz, Klima und resiliente Infrastrukturen.'),
...
VALUES
('ETOS-REF-001','Civilian Research Network Model','KONZEPT','2026-01-15',NULL,'Referenzmodell für ein ziviles, internationales Forschungs- und Innovationsnetzwerk.'),
('ETOS-REF-002','Knowledge Systems and Education','PLANUNG','2026-04-01','2027-03-31','Konzeption von Wissenssystemen für wissenschaftliche Kommunikation und Bildung.'),
('ETOS-REF-003','Photonics and Metrology Exchange','AKTIV','2026-02-01','2026-12-31','Referenzprojekt für den Austausch von Methoden in Photonik und Messtechnik.'),
('ETOS-REF-004','Sustainable Research Infrastructure','AKTIV','2026-03-15','2027-06-30','Modellierung nachhaltiger Forschungs- und Dateninfrastrukturen.')
...
UNION ALL
SELECT 'Wissenssysteme und Bildungsarchitektur','FORSCHUNGSBERICHT','REVIEW',NULL,project_id FROM etos.project WHERE project_code = 'ETOS-REF-002'
UNION ALL
SELECT 'Grundlagen der Photonik-Messtechnik','ARTIKEL','VEROEFFENTLICHT','2026-05-20',project_id FROM etos.project WHERE project_code = 'ETOS-REF-003';
...
UNION ALL
SELECT 'Photonics and Metrology Exchange','Leuven','BE','2026-09-10',project_id FROM etos.project WHERE project_code = 'ETOS-REF-003'
UNION ALL
SELECT 'Sustainable Research Infrastructure','Stockholm','SE','2026-11-05',project_id FROM etos.project WHERE project_code = 'ETOS-REF-004';
...
UNION ALL
SELECT 'Education Systems Pilot','Reference funding scenario',85000,'BEANTRAGT',project_id FROM etos.project WHERE project_code = 'ETOS-REF-002'
UNION ALL
SELECT 'Photonics Methods Programme','Reference funding scenario',140000,'BEWILLIGT',project_id FROM etos.project WHERE project_code = 'ETOS-REF-003';

INSERT INTO etos.federation_axis (federation_id, axis_id, relevance)
SELECT f.federation_id, a.axis_id, x.relevance
FROM (VALUES
  ('DE','KNOWLEDGE',5),('DE','EDUCATION',5),('DE','DATA',4),
  ('JP','ROBOTICS',4),('JP','PHOTONICS',4),('JP','EDUCATION',3),
  ('US','SPACE',5),('US','AI',4),('US','DATA',4),
  ('NO','SUSTAINABILITY',5),('NO','METROLOGY',4),('NO','DATA',3),
  ('LU','DATA',4),('LU','SUSTAINABILITY',4),('LU','HEALTH',3),
  ('BE','PHOTONICS',5),('BE','AI',4),('BE','METROLOGY',4),
  ('ES','HEALTH',4),('ES','AI',4),('ES','SUSTAINABILITY',3),
  ('NL','SUSTAINABILITY',4),('NL','DATA',4),('NL','AI',3),
  ('DK','HEALTH',4),('DK','ROBOTICS',4),('DK','SUSTAINABILITY',3),
  ('SE','ROBOTICS',4),('SE','SUSTAINABILITY',4),('SE','PHOTONICS',3)
...
FROM (VALUES
  ('ETOS-REF-001','DE','Reference model lead'),
  ('ETOS-REF-001','JP','Network reference node'),
  ('ETOS-REF-001','US','Space research reference'),
  ('ETOS-REF-002','DE','Education systems reference'),
  ('ETOS-REF-002','DK','Learning technology reference'),
  ('ETOS-REF-003','BE','Photonics methods reference'),
  ('ETOS-REF-003','JP','Measurement reference'),
  ('ETOS-REF-004','NO','Sustainability reference'),
  ('ETOS-REF-004','SE','Infrastructure reference')
