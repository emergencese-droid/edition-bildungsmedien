-- ETOS 2.0 civilian research and innovation reference schema.
-- Public reference nodes are not evidence of partnership or endorsement.
CREATE EXTENSION IF NOT EXISTS pgcrypto;
CREATE SCHEMA IF NOT EXISTS etos;
CREATE TABLE etos.federation (
 federation_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), country_code CHAR(2) NOT NULL UNIQUE,
 country_name VARCHAR(100) NOT NULL, organisation_name VARCHAR(255) NOT NULL UNIQUE,
 headquarters_city VARCHAR(120), organisation_type VARCHAR(100) NOT NULL, website TEXT,
 founded_year INTEGER, active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE etos.research_axis (
 axis_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, axis_code VARCHAR(40) NOT NULL UNIQUE,
 axis_name VARCHAR(200) NOT NULL UNIQUE, description TEXT
);
CREATE TABLE etos.federation_axis (
 ...
 relevance SMALLINT NOT NULL DEFAULT 3, PRIMARY KEY (federation_id, axis_id),
 CONSTRAINT chk_axis_relevance CHECK (relevance BETWEEN 1 AND 5)
);
CREATE TABLE etos.project (
 project_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), project_code VARCHAR(40) NOT NULL UNIQUE,
 title VARCHAR(255) NOT NULL, project_status VARCHAR(40) NOT NULL DEFAULT 'KONZEPT',
 start_date DATE, target_date DATE, description TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE etos.project_federation (
 ...
 federation_id UUID NOT NULL REFERENCES etos.federation(federation_id) ON DELETE CASCADE,
 role_name VARCHAR(120) NOT NULL, PRIMARY KEY (project_id, federation_id)
);
CREATE TABLE etos.publication (
 publication_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title VARCHAR(500) NOT NULL,
 publication_type VARCHAR(50) NOT NULL, publication_status VARCHAR(50) NOT NULL DEFAULT 'PLANUNG',
 publication_date DATE, doi VARCHAR(255) UNIQUE, project_id UUID REFERENCES etos.project(project_id) ON DELETE SET NULL,
...
CREATE TABLE etos.lecture (
 lecture_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title VARCHAR(255) NOT NULL, city VARCHAR(120),
 country_code CHAR(2) REFERENCES etos.federation(country_code) ON UPDATE CASCADE, event_date DATE,
 project_id UUID REFERENCES etos.project(project_id) ON DELETE SET NULL
);
CREATE TABLE etos.funding_program (
 funding_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title VARCHAR(255) NOT NULL, sponsor VARCHAR(255) NOT NULL,
 amount_eur NUMERIC(18,2) NOT NULL, funding_status VARCHAR(40) NOT NULL DEFAULT 'BEANTRAGT',
 project_id UUID REFERENCES etos.project(project_id) ON DELETE SET NULL,
...
CREATE TABLE etos.kpi_measurement (
 measurement_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), metric_code VARCHAR(80) NOT NULL,
 metric_name VARCHAR(160) NOT NULL, metric_value NUMERIC(18,4) NOT NULL, metric_unit VARCHAR(40) NOT NULL,
 measured_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, source_note TEXT,
...
CREATE VIEW etos.dashboard_summary AS SELECT
